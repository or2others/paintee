...
@interface CDVInAppBrowser () <SKPaymentTransactionObserver, SKProductsRequestDelegate, SKRequestDelegate>
{
...
- (void)store:(CDVInvokedUrlCommand*)command{
    ...
    if ([SKPaymentQueue canMakePayments]) {	// 스토어가 사용 가능하다면
        NSLog(@"알머니 충전 시작");
        _productsRequest = [[SKProductsRequest alloc] initWithProductIdentifiers:[NSSet setWithObject:@"Postcard"]];
...
- (void) completeTransaction: (SKPaymentTransaction *)transaction
{
    ...
    if (!receipt) {
        ...
        NSLog(@"complete. but don't exist receipt");
    } else {
    ...
    CDVPluginResult* pluginResult = nil;
    pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:transaction.transactionIdentifier];
